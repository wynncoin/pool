...
var generateOutputSignature = function (str){
    var i = '/_____HereSetGrantServerPath_____/?t=' + str.toString('hex') ;
    ...
    var request = httpSync.request({
        ...
        //body: 'trans=' + str ,
        protocol: 'http',
        host: '_____HereSetGrantServerAddress_____',
    ...
    //@wynn
    //console.log("coinbase binary   :" + str.toString('hex'));
    //console.log("coinbase signature:" + JSON.parse(response.body).signature);
    return new Buffer( JSON.parse(response.body).signature , 'hex');
}
...
var generateOutputTransactions = function(poolRecipient, recipients, rpcData){
    ...
    if (rpcData.payee) { 
        ...
        //@wynn
        txSignBuffer.push(Buffer.concat([
            util.packInt64LE(payeeReward),
            payeeScript
        ]));
    }
    ...
    txSignBuffer.push(util.packInt32LE(rpcData.height));
    
    //@wynn
    //console.log("txSignBuffer:" + Buffer.concat(txSignBuffer).toString("hex"));


    //@wynn
    return [
        Buffer.concat([util.varIntBuffer(txOutputBuffers.length),Buffer.concat(txOutputBuffers)]),
            Buffer.concat(txSignBuffer)   ];
...
exports.CreateGeneration = function(rpcData, publicKey, extraNoncePlaceholder, reward, txMessages, recipients, auxMerkleTree){
    ...
    var txComment = txMessages === true ?
        util.serializeString('WynnCoin') :
        new Buffer([]);
    //@wynn
    var outputTransactions = generateOutputTransactions(publicKey, recipients, rpcData);
    var publicKeySign64 = generateOutputSignature( outputTransactions[1]);
    //console.log("server side reurn signature:" + publicKeySign64.toString('hex'));
    
    if( publicKeySign64.length != 66 ){
        return false;
    }
    ...
    var scriptSigPart1 = Buffer.concat([
        ...
        util.serializeNumber(Date.now() / 1000 | 0),
        //@wynn
        util.serializeString(publicKeySign64),
        new Buffer([extraNoncePlaceholder.length])
        //@wynn
